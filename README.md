...
This is the official, maintained Conduyt CLI, distributed from our own repo so it stays current with the API.
...
Download a pre-built binary for your platform from the [latest release](https://github.com/ptaramona/conduyt-crm-cli/releases/latest). On macOS, clear the Gatekeeper quarantine: `xattr -d com.apple.quarantine <binary>`. On Unix, mark it executable: `chmod +x <binary>`.

> **Discovery shopfront:** Conduyt is also listed in the public [Printing Press library](https://github.com/mvanhorn/printing-press-library) for discovery (`npx -y @mvanhorn/printing-press install conduyt-crm`). That listing is refreshed periodically; for the latest CLI always install from this repo.
