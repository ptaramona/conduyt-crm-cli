...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  conduyt-crm-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set it with: conduyt-crm-pp-cli auth set-token <token> or export CONDUYT_CRM_BEARER_AUTH=\"your-token-here\""
					report["auth_docs_url"] = "https://conduyt.app"
				} else {
					authConfigured = true
			...
			if os.Getenv("CONDUYT_CRM_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "CONDUYT_CRM_BEARER_AUTH")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CONDUYT_CRM_BEARER_AUTH")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'conduyt-crm-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
