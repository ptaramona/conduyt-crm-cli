...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set it with: conduyt-crm-pp-cli auth set-token <token> or export CONDUYT_CRM_BEARER_AUTH=\"your-token-here\""))
	}
	if len(trimmed) == 0 {
