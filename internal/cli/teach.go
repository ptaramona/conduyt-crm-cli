...
// conduyt-crm-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/ptaramona/conduyt-crm-cli/internal/learn/entities"
	"github.com/ptaramona/conduyt-crm-cli/internal/learn/lookups"
	"github.com/ptaramona/conduyt-crm-cli/internal/learn/patterns"
...
const noLearnEnvVar = "CONDUYT_CRM_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("conduyt-crm-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  conduyt-crm-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  conduyt-crm-pp-cli recall "<question>" --agent
  conduyt-crm-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  conduyt-crm-pp-cli learnings list --agent
  conduyt-crm-pp-cli learnings list --query "<substring>"
  conduyt-crm-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  conduyt-crm-pp-cli learnings forget "<question>" --resource <id>
  conduyt-crm-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  conduyt-crm-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  conduyt-crm-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
