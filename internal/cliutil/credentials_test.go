...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"CONDUYT_CRM_CONFIG",
		"CONDUYT_CRM_CONFIG_DIR",
		"CONDUYT_CRM_DATA_DIR",
		"CONDUYT_CRM_STATE_DIR",
		"CONDUYT_CRM_CACHE_DIR",
		"CONDUYT_CRM_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
		"XDG_CACHE_HOME",
		"CONDUYT_CRM_BEARER_AUTH",
	} {
		t.Setenv(name, "")
	}
	...
	return home, filepath.Join(home, ".config", "conduyt-crm-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "conduyt-crm-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("CONDUYT_CRM_BEARER_AUTH", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "conduyt-crm-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "conduyt-crm-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "conduyt-crm-pp-cli", "credentials.toml")
