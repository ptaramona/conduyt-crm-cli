...
type Config struct {
	...
	ConduytCrmBearerAuth string          `toml:"crm_bearer_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("CONDUYT_CRM_BEARER_AUTH"); v != "" {
		cfg.ConduytCrmBearerAuth = v
		cfg.markEnvOverride("ConduytCrmBearerAuth")
		cfg.AuthSource = "env:CONDUYT_CRM_BEARER_AUTH"
		cfg.CredentialSource = "env:CONDUYT_CRM_BEARER_AUTH"
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("CONDUYT_CRM_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "conduyt-crm-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ConduytCrmBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:CONDUYT_CRM_BEARER_AUTH"
		}
		return "Bearer " + c.ConduytCrmBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		ConduytCrmBearerAuth: c.ConduytCrmBearerAuth,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.ConduytCrmBearerAuth = creds.ConduytCrmBearerAuth
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.ConduytCrmBearerAuth = ""
	delete(c.envOverrides, "ConduytCrmBearerAuth")
	c.updateFileConfigField("ConduytCrmBearerAuth")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ConduytCrmBearerAuth"] {
			out.ConduytCrmBearerAuth = c.fileConfig.ConduytCrmBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ConduytCrmBearerAuth":
		c.fileConfig.ConduytCrmBearerAuth = c.ConduytCrmBearerAuth
	}
}
